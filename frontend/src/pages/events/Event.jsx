      ...
      <div className="p-14 max-xl:px-2 center flex-col">
        <div className="bg-primary-gray w-full flex justify-between items-center p-4 rounded max-lg:flex-col">
          <div className="flex flex-col">
            ...
            <p className="font-bold text-[22px]  max-md:text-md">{event?.title}</p>
          </div>
          <Link
            to={`/event/${event?.id}`}
            className="button-hover button-hover bg-primary-color text-white font-bold text-lg rounded-full px-6 py-2 max-sm:text-sm max-lg:mt-4 max-lg:w-full max-lg:text-center max-md:text-sm"
          >
            Voir le programme
          </Link>
        </div>

        <div className="w-full h-[800px] max-xl:h-[600px] max-lg:h-[400px] max-md:h-[400px] max-sm:h-[300px] my-4 max-xl:my-3 ">
          <iframe
